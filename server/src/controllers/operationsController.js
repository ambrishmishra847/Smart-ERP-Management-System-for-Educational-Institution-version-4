...
import { getAiProviderPresetList, testAiProviderConnection } from "../utils/aiRiskNarrative.js";
import { writeAuditLog } from "../utils/audit.js";
import { ROLES } from "../utils/constants.js";

const ensureNumeric = (value, fallback = 0) => Number(value ?? fallback);
const sanitizeAiProvider = (settings) => {
  const aiProvider = settings?.aiProvider || {};

  return {
    presetKey: aiProvider.presetKey || "custom",
    providerType: aiProvider.providerType || "openai-compatible",
    providerName: aiProvider.providerName || "custom-ai",
    apiUrl: aiProvider.apiUrl || "",
    model: aiProvider.model || "",
    authHeader: aiProvider.authHeader || "Authorization",
    authScheme: aiProvider.authScheme || "Bearer",
    hasApiKey: Boolean(aiProvider.apiKey),
  };
};
...
export const getGlobalSettings = async (req, res) => {
  ...
  res.json({
    ...settings.toObject(),
    aiProvider: sanitizeAiProvider(settings),
    aiProviderPresets: getAiProviderPresetList(),
  });
};
...
export const saveGlobalSettings = async (req, res) => {
  ...
  const nextAiProvider = {
    presetKey: req.body.aiProvider?.presetKey || existing?.aiProvider?.presetKey || "custom",
    providerType: req.body.aiProvider?.providerType || existing?.aiProvider?.providerType || "openai-compatible",
    providerName: req.body.aiProvider?.providerName || existing?.aiProvider?.providerName || "custom-ai",
    apiUrl: req.body.aiProvider?.apiUrl || existing?.aiProvider?.apiUrl || "",
    apiKey: req.body.aiProvider?.apiKey || existing?.aiProvider?.apiKey || "",
    model: req.body.aiProvider?.model || existing?.aiProvider?.model || "",
    authHeader: req.body.aiProvider?.authHeader || existing?.aiProvider?.authHeader || "Authorization",
    authScheme: req.body.aiProvider?.authScheme || existing?.aiProvider?.authScheme || "Bearer",
  };
  const payload = {
  ...
  await writeAuditLog(req, {
    ...
    before: existing
      ? {
          ...
          aiProvider: sanitizeAiProvider(existing),
        }
      : null,
    after: {
...
export const testGlobalAiProvider = async (req, res) => {
  const existing = await GlobalSetting.findOne();
  const testConfig = {
    presetKey: req.body.presetKey || existing?.aiProvider?.presetKey || "custom",
    providerType: req.body.providerType || existing?.aiProvider?.providerType || "openai-compatible",
    providerName: req.body.providerName || existing?.aiProvider?.providerName || "custom-ai",
    apiUrl: req.body.apiUrl || existing?.aiProvider?.apiUrl || "",
    apiKey: req.body.apiKey || existing?.aiProvider?.apiKey || "",
    model: req.body.model || existing?.aiProvider?.model || "",
    authHeader: req.body.authHeader || existing?.aiProvider?.authHeader || "Authorization",
    authScheme: req.body.authScheme || existing?.aiProvider?.authScheme || "Bearer",
  };

  try {
    const result = await testAiProviderConnection(testConfig);
    await writeAuditLog(req, {
      action: "ai-provider.test",
      entityType: "GlobalSetting",
      entityId: String(existing?._id || "global"),
      metadata: {
        providerName: testConfig.providerName,
        providerType: testConfig.providerType,
        presetKey: testConfig.presetKey,
    ...
    await writeAuditLog(req, {
      ...
      metadata: {
        ...
        presetKey: testConfig.presetKey,
        model: testConfig.model,
        message: error.message || "Unable to connect to the configured AI provider.",
      },
    });
    res.status(400).json({ message: error.message || "Unable to connect to the configured AI provider." });
  }
};
