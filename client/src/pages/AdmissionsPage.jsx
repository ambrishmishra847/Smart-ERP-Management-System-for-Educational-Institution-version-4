...
const AdmissionsPage = ({ view = "pipeline" }) => {
  ...
  const loadAdmissions = async () => {
    ...
    try {
      ...
      setAdmissions(Array.isArray(response.data) ? response.data : response.data?.rows || []);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <DataTable
          loading={loading}
          error={!loading && admissions.length === 0 ? error : ""}
          emptyMessage={isApplicationView ? "No application records match the current filters." : "No admission leads match the current filters."}
          mobileTitleKey="studentName"
          rowKey={(row) => row.id}
          columns={[
