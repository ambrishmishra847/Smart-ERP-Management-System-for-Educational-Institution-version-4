...
const UsersPage = () => {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      setUsers(Array.isArray(response.data) ? response.data : response.data?.rows || []);
      setError("");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="Students Management" subtitle="Search and manage student accounts separately for easier administration.">
        ...
        <DataTable
          loading={loading}
          error={!users.some((user) => user.role === "student") ? error : ""}
          emptyMessage="No student accounts are available yet."
          mobileTitleKey="name"
          rowKey={(row) => row.id}
          columns={[
            { key: "name", label: "Name" },
      ...
      <SectionCard title="Faculty And Staff Management" subtitle="Search and manage teachers, coordinators, officers, and operational staff.">
        ...
        <DataTable
          loading={loading}
          error={!users.some((user) => !["student", "parent-guardian"].includes(user.role)) ? error : ""}
          emptyMessage="No faculty or staff accounts are available yet."
          mobileTitleKey="name"
          rowKey={(row) => row.id}
          columns={[
            { key: "name", label: "Name" },
      ...
      <SectionCard title="Parents Management" subtitle="Linked parent and guardian visibility with contact records.">
        <DataTable
          loading={loading}
          error={!users.some((user) => user.role === "parent-guardian") ? error : ""}
          emptyMessage="No parent or guardian accounts are linked yet."
          mobileTitleKey="name"
          rowKey={(row) => row.id}
          columns={[
            { key: "name", label: "Name" },
