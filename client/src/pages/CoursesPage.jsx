...
const CoursesPage = ({ view = "overview" }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {loading ? (
          <ContentState tone="loading" title="Loading course catalog" description="Courses, materials, and subject ownership are being loaded." />
        ) : error && !courses.length ? (
          <ContentState tone="error" title="Unable to load course catalog" description={error} />
        ) : !courses.length ? (
          <ContentState tone="empty" title="No courses available yet" description="Create a course or import one in bulk to start assigning classes and materials." />
        ) : (
      ...
      {!isMaterialsView ? (
        ...
        {loading ? (
          <ContentState tone="loading" title="Loading subject catalog" description="Subject mappings are being prepared for this view." compact />
        ) : !subjects.length ? (
          <ContentState tone="empty" title="No subjects available yet" description="Create subjects once courses and faculty assignments are ready." compact />
        ) : (
        <div className="grid gap-4 lg:grid-cols-2">
