  ...
  return (
    <div className="space-y-6">
      {hasPermission(user, PERMISSIONS.ANNOUNCEMENTS_MANAGE) ? (
        <BulkImportCard
          title="Bulk Announcement Import"
          subtitle="Upload notice circulars and role-targeted communication from office exports."
          target="announcements"
          sampleHeaders={["title", "content", "priority", "audience"]}
          helperText="Administration teams can preview announcement batches before publishing. Audience roles can be provided as comma-separated values like student, faculty-professor, or parent-guardian."
      ...
      <SectionCard title="Announcements" subtitle="Institution messaging for faculty, students, and administration.">
        {loading ? (
          <ContentState tone="loading" title="Loading announcements" description="The latest institutional notices are being prepared." />
        ) : error && !items.length ? (
          <ContentState tone="error" title="Unable to load announcements" description={error} />
        ) : !items.length ? (
          <ContentState tone="empty" title="No announcements available" description="Published notices and circulars will appear here once they are created." />
        ) : (
